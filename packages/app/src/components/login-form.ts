...
export class LoginFormElement extends HTMLElement {
  ...
  constructor() {
    ...
    shadow(this).styles(LoginFormElement.styles).replace(LoginFormElement.view);
  ...
  setMessage(kind: "success" | "error", text: string) {
    const root = this.shadowRoot;
    if (!root) return;
  ...
  submitForm(event: Event, endpoint: string) {
  ...
  messageForStatus(status: number, message: string, endpoint: string) {
    const isRegister = endpoint.includes("/register");
